#!/usr/bin/env python3
"""Adaptive Arb certificate for monotone transfer from the pole boundary.

The script certifies

    A_lambda''(w) = d Psi_lambda(w)/dw < 0

on one exact rational (w,lambda) rectangle. Combined with a positive boundary
anchor Phi(lambda)=lim_{w->1^-}Psi_lambda(w), this gives

    Psi_lambda(w) >= Phi(lambda) > 0

throughout the certified pole strip.
"""
...
def certify_block(
    ...
    return {
        "status": status,
        "scope": (
            f"A_lambda''(w)<0 for {w_lo}<=w<={w_hi}, "
            f"{lambda_lo}<=lambda<={lambda_hi}"
        ),
        "derived_transfer": (
            "If the positive pole-boundary anchor is available on the same "
            "lambda block, then Psi_lambda(w)>=Phi(lambda)>0 on this strip."
        ),
        "environment": {
        ...
        "terminal": terminal,
        "limitations": (
            "The inherited second-derivative kernel is not regular at the exact "
            "corner w=c=1. A terminal box touching w=1 identifies the remaining "
            "blow-up calculation rather than proving the full pole cap."
        ),
    }
